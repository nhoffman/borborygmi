...
end of 2013. There isn't a whole lot more that I need or use on my
local machine.
...
Distribute and pip (see http://www.pip-installer.org/en/latest/installing.html)
...
pip install git+ssh://git@github.com/nhoffman/csvkit.git
# for flymake-mode
pip install pep8 pyflakes flake8 autopep8
pip install pandas   # installs numpy
#+END_SRC
...
#+BEGIN_SRC sh
cd ~/src
wget http://cran.us.r-project.org/bin/macosx/R-3.1.0-mavericks.pkg && \
sudo installer -pkg R-3.1.0-mavericks.pkg -target /
#+END_SRC
...
#+BEGIN_SRC sh
sudo R --slave << EOF
packages <- c("ape", "sqldf", "ROCR", "lattice", "RSQLite", "latticeExtra")
