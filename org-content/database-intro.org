...
writing queries. For example, how can we find values greater than 10 in this table?
...
This is great for some purposes: is allows us to follow values of
these two tests for this one patient over time. However, this is a
lousy format for storing or manipulating data:

- this representation is likely to be *sparse*, where not all cells
  contain data, and requiring as many columns as there are dates in
  the entire data set.
- Adding data for an addition test is cumbersome.
...
as adding rows to the bottom. We can annotate values with arbitrary
attributes, like methods or comments. It's even easy to aggregate data
from multiple patients by adding another column.
